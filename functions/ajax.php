...
function DF_customFShare() {
	$like_count = get_transient(THEME_NAME.'_facebook_likes');
	if(!$like_count) {
		$link = $_POST['link'];
	    $like_array = json_response('http://graph.facebook.com/fql?q=SELECT%20url,%20share_count%20FROM%20link_stat%20WHERE%20url="'.esc_url($link).'"');
	    if($like_array!=false) {
		...
		set_transient(THEME_NAME.'_facebook_likes', $like_count, 600 );
	}
	echo intval($like_count);
	die();
}
...
<?php esc_html_e( 'Name:' , THEME_NAME );?> <?php echo esc_html($u_name);?><br/>
<?php esc_html_e( 'Url:' , THEME_NAME );?> <?php echo esc_url($url);?><br/>
<?php esc_html_e( 'E-mail:' , THEME_NAME );?> <?php echo sanitize_email($email);?><br/>
<?php esc_html_e( 'IP Address:' , THEME_NAME );?> <?php echo esc_html($ip);?><br/>
</div>
<?php
