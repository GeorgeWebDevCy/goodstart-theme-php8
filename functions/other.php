...
function different_theme_active() {
	return true;
}
 ...
 * -------------------------------------------------------------------------*/	
	...
	$my_meta->addSelect(THEME_NAME.'_blogStyle',array('1'=>'Grid View','2'=>'List With Small Images','3'=>'List With Large Images','4'=>'Timeline'),array('name'=> esc_html__('Category Style ',THEME_NAME), 'std'=> array('1')));
 ...
 * -------------------------------------------------------------------------*/
 ...
 function df_image_html($id, $width=0, $height=0, $class=false) {
 	...
 	if($image["show"]!=false) {
		$return = '<img'.$class.' src="'.esc_url($image["src"]).'" alt="'.esc_attr(get_the_title($id)).'" width="'.$width.'" height="'.$height.'"/>';
 	} else {
 		$return = false;
 	}
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $current_object_id = 0) {
		...
		if(($parent_menu_type=="2") && ($depth==0)) {
		   ...
		   // $item_output .= '</span>';
	        ...
	        $item_output.= $indent . '<ul class="cs_mega_menu"'.$this->set_style().'>';
...
function df_page_title() {
	...
	if(!is_archive() && !is_category() && !is_search() && $post_type!=DF_POST_GALLERY && $post_type!=DF_POST_PORTFOLIO) {
		...
		$title = esc_html__("Search Results for", THEME_NAME)." \"".esc_html($_GET['s'])."\"";
	} else if(is_category()) {
...
$differentthemes_buttons=array("differentthemessubtitle","differentthemesbutton", "differentthemesspacer", "differentthemesquote","differentthemesblocktext", "|", "|",
			 "differentthemeslists", "|","differentthemesvideo" ,"differentthemesmarker","differentthemestabs","differentthemesdropcaps", "|",
			 "differentthemescaption", "|", "differentthemesparagraph", "differentthemesparagraph2", "differentthemesparagraph5", "differentthemesparagraph3", "differentthemesparagraph4", "differentthemesalert", "differentthemesaccordion", "|", "differentthemesservice", "differentthemespricing", "differentthemestestimonials", "differentthemesbreak");

//add gallery shortcode if gallery plugin is active
if(function_exists('different_theme_gallery_active')) {
	array_push($differentthemes_buttons, 'differentthemesgallery');
}
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<article<?php if($class) echo ' class="'.$class.'"';?>>
			...
			<div class="comment_content">
                <div class="meta">
                    <span class="comment_author"><a href="<?php if(get_comment_author_url()) { echo esc_url(get_comment_author_url());} else { echo "#"; } ?>"><?php printf(esc_html__('%1$s', THEME_NAME), get_comment_author());?></a></span>
                    <span class="comment_date"><?php printf(esc_html__(' %1$s, at %2$s', THEME_NAME), get_comment_date("F d"), get_comment_time("H:i"));?></span>
                </div>
                <p><?php esc_html_e(get_comment_text()); ?></p>
                <?php comment_reply_link(array_merge( $args, array('depth' => $depth, 'max_depth' => $args['max_depth'], 'reply_text' => ''.( esc_html__( 'Reply' , THEME_NAME )).' <i class="fa fa-angle-right"></i>'))) ?>
