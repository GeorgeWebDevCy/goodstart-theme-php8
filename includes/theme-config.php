...
';
			...
			echo balanceTags($this->before_item).'
						...
						<ul class="columns block-columns clearfix inactive-columns" id="available-homepage-layouts">
							<li class="inactive-column column layout" data-id="homepageLayout_1">
								<div class="paragraph-row">
									...
									<div class="column8 column-content is-content" data-type="column8"><ul class="pagebuilder-column-container"></ul><a href="javascript:void(0);" class="pagebuilder-block-popup-open">'.esc_html__("Click to add block layout", THEME_NAME).'</a></div>
								</div>
							</li>
						...
						<li class="inactive-block component" rel="'.$block['type'].'" title="'.$block['title'].'">
							<div class="blocks-content clearfix" title="'.$block['title'].'">
								...
								if(isset($block['image'])) {
									echo '<img src="'.esc_url(THEME_IMAGE_CPANEL_URL.$block['image']).'" alt="'.esc_attr($block['title']).'" class="feature-img">';
								}
				...
				echo'
					...
					<h2><?php esc_html_e("Active homepage blocks", THEME_NAME);?></h2>
						...
						?>
									<li class="active-column column<?php if ((strpos($columRows->row,'homepageLayout') !== false)) { echo ' layout'; }?>" data-id="<?php echo esc_attr($columRows->row);?>">
										<div class="paragraph-row">
						<?php
										//foreach column rows
										$rowCount = 0;
						...
						?>
													<li class="active-block component dropped" rel="<?php echo esc_attr($rowBlock->blocksContentName);?>" style="opacity: 1; z-index: 0; ">
														<div class="blocks-content clearfix">
															...
															<?php if(isset($image)) { ?>
																<img src="<?php echo esc_url(THEME_IMAGE_CPANEL_URL.$image);?>" alt="<?php echo esc_attr($optTitle);?>" class="feature-img">
															<?php } ?>
															<div></div>
	...
	function print_export_content($value) {
		...
		<div class="input-item-full-width clearfix">
			<label><?php echo esc_html($value['title']);?></label>
				<span>
					<div class="export">
						<span id="<?php echo esc_attr($value['id']);?>" class="action export-content" data-type="<?php echo esc_attr($value['section']);?>" style="width:100px; float:right; z-index:500;"><?php esc_html_e("Export", THEME_NAME);?></span>
					</div>
				</span>
			</div>
		<?php 
	...
	*/
	function print_import_content($value) {
		...
		<div class="input-item-full-width clearfix">
			...
			<span>
				<div class="export">
					<span id="<?php echo esc_attr($value['id']);?>_button" class="action import-content" data-type="<?php echo esc_attr($value['section']);?>" style="width:100px; float:right; z-index:500;"><?php esc_html_e("Import", THEME_NAME);?></span>
			...
			?>
					<span>
						<div class="uploader">
							...
							<span id="<?php echo esc_attr($value['id']);?>_button" class="action btn-upload df-upload-button" style="width:100px; float:right; z-index:500;"><?php esc_html_e("Choose File", THEME_NAME);?></span>
						</div>
					</span>
				</div>
			<?php 
	...
	*/
	function print_checkbox($value) {
		...
		if(isset($value['page_type']) && df_page_type_check($value['page_type'])==true || !isset($value['page_type'])) { //meta box check
			...
			<?php } ?>
				<label><?php echo esc_html($value['title']);?></label>	
				<span><input type="checkbox" name="<?php echo esc_attr($value['id']);?>" id="<?php echo esc_attr($value['id']);?>"  value="on" <?php echo esc_html($checked);?> class="page-builder-input"/></span>			
			</div>		
			<?php	
			}		
		}		
			...
			<?php } ?>
					...
					<span class="textarea">
						<?php if( isset($value['upload']) && $value['upload'] == "yes" ) { ?>
							<span id="<?php echo esc_attr($value['id']);?>_button" class="action btn-upload df-upload-button" style="width:100px; float:right; z-index:500;"><?php esc_html_e("Add Media", THEME_NAME);?></span>
						<?php } ?>
			...
			if(isset($value['page_type']) && df_page_type_check($value['page_type'])==true || !isset($value['page_type'])) { //meta box check
				...
				?>
					<div class="input-item-half-width clearfix">
						...
						<span><input<?php echo esc_html($attr);?> type="radio" name="<?php echo esc_attr($value["id"]);?>" id="<?php echo esc_attr($value["id"].'_'.$n);?>"  value="<?php echo esc_attr($radio["value"]);?>" <?php echo esc_html($checked);?> class="page-builder-input"/></span>
					</div>	
				<?php
					}
					if($attr!=false) {
			...
			?>
					<li class="row-item-full-width clearfix" id="recordsArray_<?php echo convert_to_class($sidebar); ?>" alt="<?php echo esc_attr($sidebar); ?>">
						<div class="blocks-content clearfix" style="text-align: left;">
							<?php esc_html_e("Sidebar name:", THEME_NAME);?> <b><?php echo esc_html($sidebar); ?></b><a href="javascript:{}" class="button edit sidebar-edit" id="edit-<?php echo convert_to_class($sidebar); ?>" rel="<?php echo esc_attr($sidebar); ?>"><?php esc_html_e("Edit", THEME_NAME);?></a><a href="javascript:{}" class="button delete sidebar-delete" id="delete-<?php echo convert_to_class($sidebar); ?>"><?php esc_html_e("Delete", THEME_NAME);?></a>
						</div>
					</li>
			<?php
				}	
	...
	*/
	...
	?>
		<div class="row">
			<input type="hidden" name="action" value="save" /><a href="javascript:{}" class="button-1 df-save-management" data-saved="<?php echo esc_attr($value['title']);?>" data-loading="<?php esc_html_e("Saving...", THEME_NAME);?>"><?php echo esc_html($value['title']);?></a>
		</div>
	<?php
	}
