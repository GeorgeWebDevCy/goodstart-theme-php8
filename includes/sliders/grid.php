<?php
	...
	$bgType = get_post_meta ( DF_page_id(), "_".THEME_NAME."_grid_bgType", true ); 
	$color = get_post_meta ( DF_page_id(), "_".THEME_NAME."_grid_color", true ); 
	$image = get_post_meta ( DF_page_id(), "_".THEME_NAME."_grid_image", true ); 
...
?>
<!-- ======== GRID BLOCK ARTICLES ======== -->
            <div class="grid_block light"<?php echo stripslashes($sytle);?>>
                <div class="container">
                    <div class="grid_block_articles">
		        	<?php if ( $the_query->have_posts() ) : while ( $the_query->have_posts() ) : $the_query->the_post(); ?>
		        		<?php 
		        			if($counter==1) {
		        				$image = get_post_thumb($the_query->post->ID,720,605,THEME_NAME.'_homepage_image');  
		        			} else {
		        				$image = get_post_thumb($the_query->post->ID,360,307,THEME_NAME.'_homepage_image');  	
		        			}
		        		...
		        		?>
	                   	...
	                   	<?php $counter++; ?>
						<?php endwhile; else: ?>
							<p><?php  esc_html_e('No posts where found, please edit a post and set it as main slider/grid post.', THEME_NAME); ?></p>
						<?php endif; ?>
                    </div>
                </div>
            </div>
	...
	<?php } else {
			esc_html_e("Please select categories by editing the page", THEME_NAME);
		}
	?>
